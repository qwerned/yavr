...
do {  // approvedConversions — а это по-прежнему переводится
        ...
        expect(apply("новый скилл"), "новый skill")
...
do {  // realWorldLeftovers
        ...
        expect(apply("промт для клода"), "prompt для Claude")
        expect(apply("добавил раг с эмбеддингами"), "добавил RAG с embedding")
    }

do {  // yoNormalization
        expect(apply("новый промпт готов"), "новый prompt готов")
        // ё в тексте нормализуется при сравнении
